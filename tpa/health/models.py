...
class IdealWeight(models.Model):
    ...
    sex = models.CharField(
        u'Sex', max_length=1, choices=SEX_CHOICES, primary_key=True
    )

    height = models.PositiveSmallIntegerField(u'Height', help_text=u'In cm.')

    chest_measure = models.PositiveSmallIntegerField(
        u'Chest measure', help_text=u'In cm.'
    )

    slim_measure = models.PositiveSmallIntegerField(
        u'Slim measure', help_text=u'In cm.'
    )

    hip_measure = models.PositiveSmallIntegerField(
        u'Hip measure', help_text=u'In cm.'
    )

    max_weight = models.SmallIntegerField(
        u'Maximum weight', help_text=u'In Kg.'
    )

    min_weight = models.SmallIntegerField(
        u'Minimum weight', help_text=u'In Kg.'
    )
...
class PyramidGroup(models.Model):
    ...
    contraindications = models.CharField(
        u'Contraindications', max_length=255, blank=True, null=True
    )
